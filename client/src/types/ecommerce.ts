...
export type ApiOrder = { id: string; _id?: string; orderNumber: string; user: string; customerName: string; customerEmail: string; items: Array<{ product: string; name: string; price: number; quantity: number; imageUrl?: string }>; totalAmount: number; shippingAddress: string; paymentMethod: "Card" | "Cash on Delivery"; status: OrderStatus; createdAt: string };
