...
import type { StoredOrder } from "../types/ecommerce";
...
function readOrders() {
  try { return JSON.parse(localStorage.getItem("shopsphere-orders") || "[]") as StoredOrder[]; } catch { return []; }
}
...
export default function AdminDashboard() {
  ...
  const customerCount = new Set(orders.map((order) => order.address.trim().toLowerCase()).filter(Boolean)).size;
  ...
  return <main className="min-h-screen bg-[#09090f] px-4 py-8 text-white sm:px-6 lg:px-8 lg:py-10">
    <div className="mx-auto max-w-[1440px]">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricCard label="Total Orders" value={orders.length.toLocaleString("en-IN")} hint="All time orders" tone="cyan" icon={<CartIcon />} />
        ...
        <MetricCard label="Out Of Stock" value={loadingDashboard ? "—" : outOfStockCount.toLocaleString("en-IN")} hint={outOfStockCount ? "Needs your attention" : "Everything is available"} tone="red" icon={<AlertIcon />} />
      </div>
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[1.35fr_1fr]">
        <DashboardPanel title="Recent Orders" subtitle="Your latest customer purchases" action={orders.length ? `${orders.length} total` : undefined}>
          {orders.length ? <div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left text-sm"><thead className="border-b border-white/10 text-[11px] uppercase tracking-wider text-slate-500"><tr><th className="px-2 py-3 font-semibold">Order</th><th className="px-2 py-3 font-semibold">Items</th><th className="px-2 py-3 font-semibold">Date</th><th className="px-2 py-3 text-right font-semibold">Amount</th><th className="px-2 py-3 text-right font-semibold">Status</th></tr></thead><tbody className="divide-y divide-white/[0.06]">{orders.slice(0, 6).map((order) => <tr key={order.id} className="transition hover:bg-white/[0.02]"><td className="px-2 py-4 font-semibold text-slate-200">#{order.id.replace("SS-", "")}</td><td className="px-2 py-4 text-slate-400">{order.items.length} item{order.items.length === 1 ? "" : "s"}</td><td className="px-2 py-4 text-slate-400">{formatDate(order.placedAt)}</td><td className="px-2 py-4 text-right font-semibold text-white">{money(order.total)}</td><td className="px-2 py-4 text-right"><StatusBadge status={order.status} /></td></tr>)}</tbody></table></div> : <EmptyPanel icon={<CartIcon />} message="Orders will appear here after your first checkout." />}
        </DashboardPanel>
...
function StatusBadge({ status }: { status: string }) { return <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${status === "Delivered" ? "bg-emerald-400/10 text-emerald-300" : status === "Processing" ? "bg-amber-400/10 text-amber-200" : "bg-cyan-400/10 text-cyan-300"}`}>{status}</span>; }
