...
export default function HomepageSidebar(props: HomepageSidebarProps) {
  ...
  return <aside className="h-fit rounded-2xl border border-gray-800 bg-[#111118] p-4 shadow-xl shadow-black/10 lg:sticky lg:top-28">
    <div className="flex items-center justify-between"><h2 className="text-base font-bold">Filters</h2><button type="button" onClick={props.onReset} className="text-sm font-semibold text-cyan-400 transition hover:text-cyan-200">Reset</button></div>
    <label className="mt-4 block text-sm font-medium text-gray-300">Search<input value={props.search} onChange={(event) => props.onSearchChange(event.target.value)} type="search" placeholder="Search products" className="mt-2 w-full rounded-xl border border-gray-700 bg-[#0A0A0F] px-3 py-2 text-sm text-white placeholder:text-gray-500 outline-none transition focus:border-cyan-400" /></label>
...
function FilterGroup({ title, children }: { title: string; children: ReactNode }) { return <div className="mt-3 border-t border-gray-800 pt-3"><h3 className="text-xs font-semibold uppercase tracking-wide text-gray-300">{title}</h3><div className="mt-1 space-y-0.5">{children}</div></div>; }
